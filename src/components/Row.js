...
export default function Row({ title, isLargeRow, movies, favorite, handleFave, base_url }) {
    return (
        <div className="row">
            <h2>{title}</h2>
            {/* title */}
            <div className={"row_posters" + (isLargeRow ? ' row_large' : '')}>
                {movies.map(movie => (
                    ...
                    (
                        <div className={"poster_image" + (isLargeRow ? ' poster_large' : ' poster_small')} key={movie.id}>
                            <img className={"row_poster" + (isLargeRow ? " row_posterLarge" : " row_posterSmall")}
