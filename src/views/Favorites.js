...
export default function Favorites({ findfavorite, baseUrl, handleFave, favorite }) {
  return (
    <main className="favorites_container">
      <h1>Favorites</h1>
      {findfavorite.length > 0 ? (<div className="row_favorite">
        {findfavorite.map(movie => (
          movie.vote_average != '' && movie.poster_path && movie.backdrop_path &&
          (
            <div className={"poster_image" + ' poster_small'} key={movie.id}>
              <img className={"row_poster" + " row_posterSmall"}
                src={`${baseUrl}${movie.backdrop_path}`} alt={movie.name}
                onClick={() => handleFave(movie.id)}
              />
              <div className="info">
                <div className="rating">Rating: {movie.vote_average} </div>
                <div className="title">{movie?.name || movie?.title || movie?.original_name}</div>
                <div onClick={() => handleFave(movie.id)} className="thumbnail_icons_actions"> <svg
                  id="heart"
      ...
      </div>) : (<div> No favorites has been added.</div>)
      }
    </main>
  );
}
